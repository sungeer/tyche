...
async def run(state, sse_queue=None):
    ...
    try:
        ...
        if _should_short_circuit(state):
            ...
            _push_short_circuit_response(sse_queue, state)
            return state

        # 分支 意图不明确 返回澄清问题
        if state['working']['intent']['needs_clarification']:
            state['control']['status'] = 'completed'
            _push_working_response(sse_queue, state)
            return state

        # Node2 合规拦截 与 Skill 路由
        state['control']['current_node'] = 'node2'
        state = await node2_compliance_gate.run(state)
        await step_log.write_step(state, 'node2_compliance_gate')
        ...
        if state['working']['route']['human_review']['required']:
            ...
            state['control']['review_task_id'] = task_id
            _push_pending_review(sse_queue, task_id, review_info['sla_hours'])
            asyncio.create_task(node5_audit_writer.run(state))
            return state

        # Node3 Skill 执行 与 RAG 检索
        state['control']['current_node'] = 'node3'
        state = await node3_skill_executor.run(state)
        await step_log.write_step(state, 'node3_skill_executor')

        # 分支 写操作 Skill 关键失败 → 整体失败
        if _is_critical_failure(state):
            state['control']['status'] = 'failed'
            _push_error(sse_queue, '关键操作执行失败，请稍后重试')
            asyncio.create_task(node5_audit_writer.run(state))
            return state

        # Node4 响应合成 LLM SSE 流式
        state['control']['current_node'] = 'node4'
        state = await node4_response_synthesizer.run(state, sse_queue=sse_queue)
        await step_log.write_step(state, 'node4_response_synthesizer')
        ...
        # 节点在抛异常前已向 node_traces 追加 failed 轨迹，此处补写步骤记录
        if state['audit']['node_traces']:
            failed_node = state['audit']['node_traces'][-1]['node']
            await step_log.write_step(state, failed_node)
        _push_error(sse_queue, '服务暂时不可用，请稍后重试')
        asyncio.create_task(node5_audit_writer.run(state))

    finally:
        # 更新 workflow_run 最终状态，再关闭 SSE 流
        await step_log.finish_run(state)
        _signal_sse_done(sse_queue, state)
...
def _push_working_response(queue, state):
    if not queue:
        return
...
# 闲聊或超出范围的 礼貌提示
def _push_short_circuit_response(queue, state):
    if not queue:
        return
    category = state['working']['intent']['category']
...
def _push_pending_review(queue, task_id, sla_hours):
    """人工审核等待中的提示"""
    if not queue:
        return
...
def _signal_sse_done(queue, state):
    """
