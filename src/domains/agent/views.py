...
async def chat(request):
    ...
    # 保存 state 引用，Pipeline 完成后保存消息
    turn_state_holder.append(state)
...
async def review_list(request):
    ...
    # 从用户角色中提取审核角色
    # 约定：JWT roles 中包含 'COMPLIANCE'、'RISK_OFFICER' 等表示审核角色
    review_roles = [r for r in user.roles if r in ('COMPLIANCE', 'RISK_OFFICER', 'ADMIN')]
    if not review_roles:
        return fail(403, '当前角色无权查看审核任务')

    role = review_roles[0]
    tasks = await service.get_review_list(role)
...
async def session_clear(request):
    ...
    success = await service.clear_session(user.user_id, session_id)
    if not success:
...
async def metrics(request):
    ...
    if 'skill:manage' not in user.roles:
        return fail(403, '无权访问系统指标，需要 skill:manage 权限')
